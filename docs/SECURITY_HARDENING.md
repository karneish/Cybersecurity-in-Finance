...
- [ ] Changed all demo user passwords (`scro_regulator`, `scro_banker`, `scro_auditor`)
- [ ] Removed or rotated the `Scro@2026!` default demo password
...
- [x] Refresh token reuse detection is active (default; do not disable)
- [x] Role-based access is enforced server-side (not just frontend)
...
- [x] Migrations have been run (`python database/migrate_and_seed.py`)
- [ ] Unused schemas are not accessible outside the application

## Logging and monitoring

- [ ] Container logs are shipped to a centralised logging system
- [x] Failed login attempts are monitored (written to `auth.audit_logs`)
- [ ] Audit chain entries are verified periodically (`POST /api/risk/audit/verify`)

## Dependency management

- [x] Dependabot is enabled (see `.github/dependabot.yml`)
- [x] Security scan CI workflow runs weekly (see `.github/workflows/security-scan.yml`)
